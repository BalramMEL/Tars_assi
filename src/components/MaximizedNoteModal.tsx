...
export const MaximizedNoteModal = ({ isOpen, onClose, content, onChange }: MaximizedNoteModalProps) => {
  ...
  const handleContentChange = (e: React.FormEvent<HTMLDivElement>) => {
    ...
    const customEvent = {
      target: { name: "noteContent", value: newContent },
      ...
      <DialogContent className="max-w-4xl">
        ...
        <div className="relative">
          ...
          <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex items-center justify-center space-x-2 p-1 border border-gray-200 bg-white rounded-lg shadow-md w-max">
            <Button variant="ghost" size="icon" onClick={() => handleTextFormat("bold")} title="Bold (Ctrl+B)" className="w-10 h-10">
              <Bold className="h-5 w-5" />
            </Button>
            <Button variant="ghost" size="icon" onClick={() => handleTextFormat("italic")} title="Italic (Ctrl+I)" className="w-10 h-10">
              <Italic className="h-5 w-5" />
            </Button>
            <Button variant="ghost" size="icon" onClick={() => handleTextFormat("underline")} title="Underline (Ctrl+U)" className="w-10 h-10">
